...
echo "  • Start the newspaper UI:    ${C_BOLD}cd ${INSTALL_DIR} && npm run dev${C_RESET}   (http://localhost:3000)"
echo "  • Or with Docker (UI only):  ${C_BOLD}cd ${INSTALL_DIR} && docker compose up -d${C_RESET}"
echo "  • Open a Claude Code session anywhere — activity will start showing up"
echo ""
